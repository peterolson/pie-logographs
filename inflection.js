...
function convertPIEToHiragana(text) {
    const phonemes = Object.keys(toHiragana).sort(
        (a, b) => b.length - a.length
    );
    let outText = "";
    let i = 0;
    while (i < text.length) {
        const phoneme = phonemes.find((x) => text.startsWith(x, i));
        if (!phoneme) {
            console.error("No phoneme found for", text.slice(i));
            return outText;
        }
        outText += toHiragana[phoneme];
        i += phoneme.length;
    }
    return outText;
}

function renderHiragana(suffix) {
    if (suffix.root) {
        return lexicon[suffix.root];
    }
    if (suffix.combo) {
        const parts = [];
        for (const item of suffix.combo) {
            if (item in lexicon) {
                parts.push(lexicon[item]);
            } else if (item in suffixes) {
                parts.push(renderHiragana(suffixes[item]));
            }
        }
        return parts.join("");
    }
    if (suffix.alias) {
        return renderHiragana(suffixes[suffix.alias]);
    }
    if (suffix.shortForm) {
        return convertPIEToHiragana(suffix.shortForm);
    }
    return convertPIEToHiragana(suffix.form);
}
...
export function addInflection(word) {
    if (word.pos === "adj") {
        if (
            word.gender === "m" &&
            word.number === "sg" &&
    ...
    if (word.pos === "verb") {
        if (!word.verbType) {
            console.error("No verb type for", word.word);
        }
        if (!word.formation && !word.suffix) {
            console.error("No formation for", word.word);
        }
